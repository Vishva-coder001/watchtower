...
app.post("/sites/:siteId/run", async (req, res) => {
  ...
  try {
    ...
    console.log(
      `Scrape successful: ${snapshot.id} | ${changeStatus}`
    );
  ...
  try {
    ...
    for (const site of sites) {
      ...
      try {
        ...
        console.log(
          `Scrape successful: ${site.name} | ${changeStatus}`
        );
...
app.listen(PORT, () => {
  console.log(
    `Scraper runner listening on http://localhost:${PORT}`
  );
});
